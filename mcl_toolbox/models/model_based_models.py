...
class ModelBased(Learner):
    ...
    @staticmethod
    def get_myopic_action(list_of_lists):
        max_value = float('-inf')  # Initialize with negative infinity
        max_index = None

        for index, sublist in enumerate(list_of_lists):
            current_max = max(
                sublist)  # todo: do we need to break ties, i.e. randomly choose if there are ties? Otherwise max always takes the first action
    ...
    def get_action(self, trial_info):
        ...
        if self.compute_likelihood:
            ...
            best_action = self.get_myopic_action(expected_return)
            action = best_action
        return action
