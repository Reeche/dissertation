    ...
    def run_multiple_simulations(
        ...
        for _ in range(num_simulations):
            trials_data = self.simulate(
                ...
                apply_microscope=apply_microscope,
            )
...
class BaseRSSL(Learner):
    ...
    def gaussian_max_likelihoods(self):
        ...
        likelihoods = [
            mp.quad(lambda x: integrate(x, i, means, sigmas), [min_val, max_val])
    ...
    def update_bernoulli_params(self, reward, strategy_index):
        ...
        if choice:
            ...
            params[strategy_index + self.num_strategies] = (
                params[strategy_index + self.num_strategies] + 1
            )
    ...
    def update_gaussian_params(self, reward, strategy_index):
        ...
        num_strategies = self.num_strategies
        params = self.params
        params[strategy_index] = (
            params[strategy_index + num_strategies] * reward
            + params[strategy_index] * var
        ) / (params[strategy_index + num_strategies] + var)
        params[strategy_index + num_strategies] = (
            params[strategy_index + num_strategies]
            * var
            / (params[strategy_index + num_strategies] + var)
        )
    ...
    def simulate(
        self, env, all_trials_data, apply_microscope=False, compute_likelihood=False
    ):
        env.reset()
        ...
        for trial_num in range(num_trials):
            ...
            if compute_likelihood:
                ...
                actions_strategy_log_likelihood = self.get_action_strategy_likelihood(
                    trial, clicks, chosen_strategy, temperature
                )
                self.action_log_probs.append(
                    float(
                        str(
                            actions_strategy_log_likelihood
                            + mp.log(strategy_likelihood)
                        )
                    )
                )
...
class BaseREINFORCE(Learner):
    ...
    def get_action_details(self, env):
        ...
        mer = [
            present_node_map[action].calculate_max_expected_return()
    ...
    def get_current_weights(self):
        return (
            torch.squeeze(self.policy.weighted_preference.weight.data).tolist()
            + [self.beta]
            + [self.policy.as_a.data, self.policy.as_b.data]
        )
...
class BaseLVOC(Learner):
    ...
    def perform_action_updates(self, env, features, reward, term_features, term_reward):
        ...
        pr = (
            self.get_term_reward(env) - self.pr_weight * self.term_rewards[-1]
    ...
    def store_action_likelihood(self, env, given_action):
        ...
        if num_available_actions == 1:
            ...
            probs = np.array(
                [
                    mp.quad(
                        lambda x: integrate(x, i, means, sigmas), [min_val, max_val]
                    )
    ...
    # This function is only used in conjunction with the hierarchical learner
    def store_hierarchical_action_likelihood(
        self, env, new_action, reward, first_trial, first_action=False, first_path=None
    ):
        if not first_action:
            features = self.next_features
            ...
            if compute_likelihood:
                ...
                if trial_num == 0:
                    ...
                    (
                        next_action,
                        self.next_features,
                    ) = self.get_first_trial_action_details(env, action)
                else:
...
class DiscoverySelection(Learner):
    ...
    def init_ts_params(self):
        ...
        self.precision = np.zeros(
            (num_strategies, self._num_features, self._num_features)
        )
        for s_num in range(num_strategies):
            self.precision[s_num] = np.diag(
                [1 / (self._standard_dev) ** 2] * self._num_features
            )
    ...
    def get_action_details(self, env, strategy_num):
        ...
        for index, action in enumerate(available_actions):
            computed_features = current_trial.node_map[
                action
            ].compute_termination_feature_values(self._features)
            computed_features = get_normalized_feature_values(
    ...
    def get_pseudo_lvoc_log_likelihood(
        self, env, actions, rewards, strategy_num, true_update=False
    ):
        env.reset_trial()
        log_likelihoods = []
        s = strategy_num
        ...
        if true_update:
            action, f_next, log_prob = self.get_action_features_likelihood(
                env, given_action, self.mean[s], self.precision[s]
            )
        else:
            action, f_next, log_prob = self.get_action_features_likelihood(
                env, given_action, mean, precision
            )
        term_rewards.append(self.get_term_reward(env))
        episode_features.append(f_next)
        ...
        while 1:
            ...
            if not true_update:
                mean, precision, gamma_a, gamma_b = estimate_bayes_glm(
                    f, update_r + pr, mean, precision, gamma_a, gamma_b
                )
            else:
            ...
            if self._vicarious_learning:
                ...
                if not true_update:
                    mean, precision, gamma_a, gamma_b = estimate_bayes_glm(
                        term_features, term_reward, mean, precision, gamma_a, gamma_b
                    )
                else:
                    self.update_lvoc_params(strategy_num, term_features, term_reward)
    ...
    def compute_log_likelihood(self, env, actions, rewards):
        ...
        lvoc_log_likelihoods = [
            self.get_pseudo_lvoc_log_likelihood(env, actions, rewards, i)
            for i in range(num_strategies)
        ]
        # See if this is the right way to do things
        log_likelihoods = np.array(strategy_log_likelihoods, dtype=float) + np.array(
            lvoc_log_likelihoods, dtype=float
        )
    ...
    def simulate(
        ...
        for trial_num in range(num_trials):
            ...
            if compute_likelihood:
                ...
                rewards = all_trials_data["rewards"][trial_num]
                self.action_log_probs.append(
                    self.compute_log_likelihood(env, actions, rewards)
                )
                chosen_strategy = self.select_strategy()
                self.get_pseudo_lvoc_log_likelihood(
                    env, actions, rewards, chosen_strategy, True
                )
                reward = np.sum(rewards)
...
class HierarchicalAgent:
    ...
    def __init__(
        self, tau, decision_rule="threshold", decision_params=None, features=None
    ):
        self.tau = tau
        self.payoffs = []
        ...
        self.decision_params = (
            self.get_init_decision_params()
            if not self.decision_params
            else decision_params
        )
        self.previous_decision_params = get_zero_params(self.decision_params)
        self.features = features
        self.termination_features = [
            1,
            2,
            4,
            18,
    ...
    def compute_stop_prob(
        ...
        if decision_rule == "threshold":
            p_stop = temp_sigmoid(max_expected_return - decision_params["theta"], tau)
        elif decision_rule == "best_payoff":
            p_stop = temp_sigmoid(
                max_expected_return - decision_params["theta"] * max_payoff, tau
            )
        elif decision_rule == "average_payoff":
            p_stop = temp_sigmoid(
                max_expected_return - decision_params["theta"] * avg_payoff, tau
            )
    ...
    def __init__(
        ...
        **model_params,
        ...
        self.second_stage_models = {
            "lvoc": LVOC,
            "vicarious_lvoc": VicariousLVOC,
            "montecarlo_lvoc": MonteCarloLVOC,
            "termination_lvoc": TerminationLVOC,
            "termination_montecarlo_lvoc": TerminationMonteCarloLVOC,
            "delay_lvoc": DelayLVOC,
            "delay_reinforce": DelayREINFORCE,
            "cost_lvoc": CostLVOC,
            "reinforce": REINFORCE,
            "cost_delay_lvoc": CostLVOCDelay,
            "full_lvoc": FullLVOC,
            "cost_term_mc_lvoc": CostTermMCLVOC,
        }
        self.actor_agent = self.second_stage_models[self.actor](
            features=self.features,
            ...
            if compute_likelihood:
                ...
                for i in range(len(trial_actions)):
                    ...
                    if not self.no_term or action != 0:
                        ...
                        ) = self.actor_agent.store_hierarchical_action_likelihood(
                            env,
                            new_action=action,
                            reward=reward,
                            first_trial=True,
                            first_action=first_action,
                            first_path=first_trial_path,
                        )
            else:
                if trial_num == 0:
...
models = {
    ...
    "full_lvoc": FullLVOC,
    "bernoulli_rssl": BernoulliRSSL,
    "gaussian_rssl": GaussianRSSL,
    "cost_term_mc_lvoc": CostTermMCLVOC,
    "ds": DiscoverySelection,
    "vicarious_ds": VicariousDS,
    "cost_ds": CostDS,
    "montecarlo_ds": MonteCarloDS,
    "termination_ds": TerminationDS,
    "termination_montecarlo_ds": TerminationMonteCarloDS,
    "full_ds": FullDS,
    "null_lvoc": NullLVOC,
    "null_bernoulli_rssl": NullBernoulliRSSL,
    "null_gaussian_rssl": NullGaussianRSSL,
...
class ParameterOptimizer:
    ...
    def __init__(
        ...
        normalized_features=None,
        use_pseudo_rewards=False,
        optimization_criterion="performance_error",
        num_simulations=30,
        max_evals=100,
        decision_rule=None,
        second_stage_learner=None,
        optimize_priors=False,
        optimize_pr_weight=False,
        no_term=False,
        strategy_space=None,
        **params,
    ):
        ...
        self.env = GenericMouselabEnv(
            len(self.participant.envs),
    ...
    def get_hyperparameter_range(self, decision_rule):
        ...
        if decision_rule == "adaptive_satisficing":
            ...
            hp_space["theta"] = hp.loguniform("theta", np.log(0.01), np.log(1e2))
        elif decision_rule == "best_path_difference":
            hp_space["theta"] = hp.uniform("theta", -150, 150)
        elif decision_rule == "noisy_memory_best_payoff":
    ...
    def get_objective_value(self, simulations_data):
        ...
        if criterion == "reward":
            ...
            objective_value = -strategy_accuracy(p_s, a_s)[self.participant.pid]
    ...
    def get_parameter_ranges(self):
        ...
        param_ranges = {
            ...
            "subjective_cost": hp.loguniform(
                "subjective_cost", np.log(1e-2), np.log(60)
            ),
        ...
        if "rssl" in model_name:
            ...
            if "gaussian_rssl" in self.model_name:
                for i in range(num_strategies):
                    total_param_ranges[f"priors_{i}"] = hp.loguniform(
                        f"priors_{i}", -3, 0
                    )
            ...
            if self.second_stage_learner:
                ...
                second_stage_args = inspect.getfullargspec(
                    self.models[self.second_stage_learner].__init__
                ).args
            ...
            if (
                "lvoc" in second_stage_learner[-4:] or "lvoc" in model_name[-4:]
            ) or "reinforce" in model_name:
    ...
    def model_objective(self, params):
        ...
        if "rssl" in self.model_name:
            ...
            agent = model(
                ...
                use_pseudo_rewards=self.use_pseudo_rewards,
                features=self.features,
                bandit_params=bandit_params,
                strategy_weights=np.zeros((num_strategies, len(self.features))),
                normalized_features=self.normalized_features,
                **params,
            )
        else:
        ...
        if self.optimization_criterion not in [
            "strategy_accuracy",
            "strategy_transition",
            "likelihood",
        ]:
            ...
            simulations_data = agent.run_multiple_simulations(
                ...
                all_trials_data=self.all_trials_data,
    ...
    def get_best_parameters(self, trials=False):
        ...
        if trials:
            trials = Trials()
            best_params = fmin(
                fn=self.model_objective,
                space=self.parameter_ranges,
                algo=algo,
    ...
    def run(
        ...
        if "rssl" in self.model_name:
            ...
            if self.decision_rule:
                ...
                self.decision_params = {
                    k: params[k] for k in init_decision_params.keys()
                }
