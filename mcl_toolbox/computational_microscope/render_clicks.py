...
def render_sequence_clicks(trial_sequence, click_sequences, path, gif=True):
    for i, (trial, clicks) in enumerate(
        zip(trial_sequence.trial_sequence, click_sequences)
    ):
        d_path = f"{path}/{i}"
        create_dir(d_path)
...
def render_experiment_clicks(exp_num, pipeline, path, block=None):
    ...
    for pid in envs.keys():
        ...
        render_sequence_clicks(
            trial_sequence, clicks[pid], path=f"{path}/{exp_num}/{pid}"
        )
