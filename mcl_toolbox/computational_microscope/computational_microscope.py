...
from mcl_toolbox.utils.learning_utils import pickle_load, construct_reward_function, reward_levels, \
    construct_repeated_pipeline
from mcl_toolbox.utils.sequence_utils import compute_trial_features, compute_trial_feature_log_likelihood
...
class ComputationalMicroscope():
    ...
    def compute_trials_likelihood(self, click_sequences, envs):
        ...
        for trial_num in range(num_trials):
            ...
            trial_features = compute_trial_features(self.pipeline, envs[trial_num], click_sequence, self.features,
                                                    self.normalized_features)
            log_likelihoods.append([self.get_trial_log_likelihood(trial, trial_features, click_sequence, i) for i in
    ...
    def apply_microscope(self, click_sequences, envs):
        """
            ...
            Arguments:
                ...
                envs: The values under each node in the Mouselab-MDP graph.
    ...
    def infer_participant_sequences(self, pids, p_envs, p_clicks, max_evals=100, fit_strategy_temperature=True,
                                    show_pids=True):
        ...
        for pid in pids:
            ...
            if pid in p_clicks:
                ...
                if len(clicks) == 0:
                    continue
                S, _, _, T = self.infer_sequences(clicks, envs, fit_strategy_temperature=fit_strategy_temperature,
                                                  max_evals=max_evals)
                strategies[pid] = S
                temperatures[pid] = T
                print("S AND T", S, T)
