...
if __name__ == "__main__":
    ...
    v1 = Experiment(mod_exp_num, cm=cm, pids=pids, block = block, **additional_constraints)
    v1.infer_strategies(max_evals=100)
    create_dir(f"results/final_strategy_inferences/{exp_num}_{block}")
    strategies = pickle_save(v1.participant_strategies[pid], f"results/final_strategy_inferences/{exp_num}_{block}/{pid}_strategies.pkl")
    temperatures = pickle_save(v1.participant_temperatures[pid], f"results/final_strategy_inferences/{exp_num}_{block}/{pid}_temperatures.pkl")
