...
class MouselabEnv(gym.Env):
    ...
    def get_distance_matrix(self, distance_function=distance.euclidean):
        return [
            [distance_function(self.mdp_graph.nodes[first_node]["layout"], self.mdp_graph.nodes[second_node]["layout"])
             for second_node in self.mdp_graph.nodes(data=False)] for first_node in self.mdp_graph.nodes(data=False)]
    ...
    def myopic_voc(self, action, state) -> NonNegativeFloat:
        if action == self.term_action:
            ...
            corrected_gain_from_inspecting = np.sign(gain_from_inspecting) * np.abs(
                gain_from_inspecting) ** self.power_utility
    ...
    def _render(self, mode="notebook", close=False, use_networkx=False):
        ...
        if use_networkx == True:
            ...
            labels = {node: (str(self._state[node]) if not hasattr(self._state[node], 'sample') else '') for node in
                      self.mdp_graph.nodes(data=False)}
            node_color = [("#DEDEDE" if labels[node] == "" else color(float(labels[node]))) for node in
                          range(len(self.mdp_graph.nodes(data=False)))]
            plot = nx.draw(self.mdp_graph, pos=layout, labels=labels, node_color=node_color, node_size=2000,
                           arrowsize=30)
            return plot
        else:
