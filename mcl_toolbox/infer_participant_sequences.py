...
def infer_many_participant_sequences(exp_num_list = ["F1"],pid_list = [0, 3, 5, 13],  block = "training"):
    '''
...
def infer_participant_sequences(pid, exp_num, block = None):
    ...
    inferred_strategies, inferred_temperatures = infer_strategies(clicks, envs, pipeline, strategy_space,
    ...
    if not block:
        ...
        learning_utils.pickle_save(inferred_temperatures, f"{path}/{pid}_temperature.pkl")
    else:
        learning_utils.pickle_save(inferred_strategies, f"{path}/{pid}_{block}_strategies.pkl")
        learning_utils.pickle_save(inferred_temperatures, f"{path}/{pid}_{block}_temperature.pkl")
