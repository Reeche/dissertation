...
"""
Currently only tests to make sure we can instantiate a ModelFitter object
python3 -m unittest tests.test_model_fitter
"""

# here, the name is just used for loading in participant data
new_pipeline = [["F1", "high_increasing", 3, 15],
                    ["c1.1", "low_constant", 1, 15]]
old_pipeline = [["F1", "test"],
                ["c1.1","train_final"]]


class TestModelFitter(unittest.TestCase):
    @parameterized.expand(new_pipeline)
    def test_new_pipeline_with_exp(self, exp_name, exp_setting, pid, num_trials):
        # For IRL project
    ...
    @parameterized.expand(new_pipeline)
    def test_new_pipeline_with_kwargs(self, exp_name, exp_setting, pid, num_trials):
        # For new MCRL project
        mf = ModelFitter(exp_name, exp_setting=exp_setting, num_trials=num_trials)
        # reached end
        self.assertTrue(True)
    @parameterized.expand(old_pipeline)
    def test_old_pipeline_with_exp(self, exp_name, block):
