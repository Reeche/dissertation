...
def get_all_combinations(model, condition):
    # mapping = {"rl_hybrid_variants": [480, 481, 482, 483, 484, 485, 486, 487, 488, 489, 490]}
    combinations = list(itertools.product([*pid_dict[condition]], [model]))
    return combinations
...
if __name__ == "__main__":
    ...
    model_list = int(sys.argv[2])  # even if it is a single model, it has to be a list

    # model_list = [480, 481, 482, 483, 484, 485, 486, 487, 488, 489, 490]
    # model_list = 480
    ...
    combinations = get_all_combinations(model_list, condition)

    # create new dataframe with the columns "pid", "class", model_index"
    df = pd.DataFrame(columns=["pid", "model"])
    # class column are the second elements of the combinations
    df["model"] = [x[1] for x in combinations]
    ...
    for index, row in df.iterrows():
        ...
        data = pd.read_pickle(f'{root_folder}/{model}/{condition}_data/{pid}_{model}_1.pkl')
        model_params = pd.read_pickle(f'{root_folder}/{model}/{condition}_priors/{pid}_likelihood_{model}.pkl')
    ...
    df.to_csv(f"{model}_{condition}.csv")
    # print(df)
