...
def plot_model_proportion(data, participants, experiment):
    ...
    for models in model_groups.values():
        ...
        plot_all_strategy_proportions_model(data, models, experiment, plot=True)

def plot_all_strategy_proportions_model(data, models, experiment, plot=True):
    for model in models:
        print(model)
        data_filtered = data[data["model"] == model]
        # for the column "model_strategies", replace integers with strategy_mapping
        df = data_filtered["model_strategies"].apply(pd.Series)
        df = classify_strategies(df, experiment)  # <- strategy_mapping used here

        # Map numeric strategy codes to labels using strategy_mapping
        frequencies = pd.DataFrame(columns=["Adaptive", "Mod. adaptive", "Maladaptive"])
        counts_df = pd.DataFrame(columns=["Adaptive", "Mod. adaptive", "Maladaptive"])
        ...
        if plot:
            ...
            plt.ylim(-10, 100)
            plt.xlabel("Trials", fontsize=14)
            plt.ylabel(f"Proportion for {model}", fontsize=14)
            plt.legend(fontsize=14)
            plt.savefig(f"plots/{experiment}/{model}_proportions.png")
            # plt.show()
            plt.close()
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        ...
        # plot_all_strategy_proportions(participants_df, experiment, plot=True)
        ...
        plot_model_proportion(model_data, participants_df, experiment)
