...
def plot_score(click_df):
    ...
    plt.plot(pid_average, label=f"Participant: {pid_average.values[0]:.1f} to {pid_average.values[-1]:.1f}",
             color="blue")
...
def plot_confidence_interval(actual_count, prop_counts, model, participant=None):
    ...
    plt.plot(x, prop_counts, label=f"Participant: {prop_counts.values[0]:.1f} to {prop_counts.values[-1]:.1f}",
             color='blue')
    ...
    if participant:
        ...
        plt.plot(x, participant[1]["optimal_strategy"],
                 label=f"Participant: {count_prop[1]['optimal_strategy'][0]:.1f} to {count_prop[1]['optimal_strategy'][-1]:.1f}",
                 color='red')
...
if __name__ == "__main__":
    ...
    plot_combined(click_df, count_prop[0]["optimal_strategy"], count_prop[1]["optimal_strategy"], "Participant",
                  count_prop)
