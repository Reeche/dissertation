...
for model in models:
    ...
    for pkl in data_path.glob("*.pkl"):
        ...
        if not Path(f"../../data/model/variants/{dir}/{exp}").exists():
            Path(f"../../data/model/variants/{dir}/{exp}").mkdir()

    # save as csv next to "human" and data folder
    df.to_csv(f"../../data/model/variants/{dir}/{exp}/mouselab-mdp.csv", index=False)
