...
def create_dataframe_of_fitted_model_pid(exp_num, pid_list, model_list, model_information_filtered):
    ...
    df.to_csv(f"results/{exp_num}_intermediate_results_v3.csv")  # server
...
def create_bic_table(df, exp_num):
    ...
    df_pivot.to_csv(f"results/matlab_{exp_num}_bic_v3.csv")  # server
...
def create_one_bic_table():
    ...
    df = pd.read_csv(f"cogsci_analysis/results/matlab_v1.0_bic_v3.csv")

    for exp_name in exp_num_list:
        temp_df = pd.read_csv(f"cogsci_analysis/results/matlab_{exp_name}_bic_v3.csv")
        temp_df = temp_df.iloc[2:]
        df = df.append(temp_df)
    df.to_csv("cogsci_analysis/results/bic_all_v3.csv")
    return df
...
if __name__ == "__main__":
    ...
    # models that have pseudo-reward weights
