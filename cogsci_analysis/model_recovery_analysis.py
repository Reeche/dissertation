...
model_list = ["1823", "1819", "1787", "1919", "1822"]
for exp_name in exp_num_list:
    for model in model_list:
        ...
        pid_average_score = pid_df.groupby("trial_index")["score"].mean().to_frame()["score"]  # output is series(pid; score)


        # plot average
        plt.plot(model_average_score, label="Model")
        plt.plot(pid_average_score, label="Participant")
        # add 95% CI
        # ci = 1.96 * np.std(average_score)/np.sqrt(len(average_score))

        # get standard error of the models
        model_sem = stats.sem(model_score_array)
        ci = 1.96 * model_sem
        plt.fill_between(list(range(0, 35)), model_average_score-ci, model_average_score+ci, alpha=0.3, label='Model 95% CI')

        # get standard error of the models
        participant_sem = stats.sem(pid_score_array)
        ci = 1.96 * participant_sem
        plt.fill_between(list(range(0, 35)), pid_average_score-ci, pid_average_score+ci, alpha=0.3, label='Participant 95% CI')

        plt.xlabel("Trial number")
        plt.ylabel("Score")
        plt.legend()
        plt.savefig(f"results/plots/{exp_name}_{model}.png")
        plt.close()
        # plt.show()